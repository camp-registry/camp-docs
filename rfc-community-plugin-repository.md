...
- For severe cases, a signed revocation removes the affected version from installation entirely while preserving it in the archive for forensic purposes. Revocation follows the version identifier, so a revoked version is retired permanently; a fix ships as a new version.
...
| Catalog replication and enclosure (a storefront or fork replicates the index, enriches it privately, and diverges) | The index database is licensed ODbL: replication and produced works (storefronts, install tooling) are free with attribution, and wholesale derivatives of the database default to share-back. Stated honestly: ODbL raises the cost of enclosure and sets the default; it does not make enclosure impossible (a determined actor can structure enrichment as a separate database joined at query time), and in jurisdictions without database rights its force is contractual. The residual mitigations are the ones that never expire: being the canonical, freshest source, and the exit guarantee (§9). Advisories are CC BY 4.0 so security warnings propagate without friction. Contributions are accepted under CC0 (camp-index CONTRIBUTING.md), so the project can always relicense as one party. |
