...
## D29: Composer requirements are a translation of the author's declaration

**Decision.** packages.json links each package to the other camp packages it needs (camp-tools#51): the parent package for a subplugin of a third-party parent, and every `$plugin->dependencies` entry that names a plugin camp serves. The constraint is the exact list of the dependency's served versions whose recorded `$plugin->version` meets the declared minimum, regenerated on every publish. Dependencies Moodle bundles on a supported branch are omitted; ones camp does not serve, or serves only below the declared minimum, are omitted and noted in the publish log. The declared dependencies also ride in `extra.camp.dependencies`. A `--no-requirements` switch turns the linking pass off without a code change.
